...
function tvReadForm() {
    ...
    tvNormalizeSlides(s).forEach((slide) => {
        const row = document.querySelector('[data-tv-slide="' + slide.id + '"]');
        if (!row) return;
        const nameInput = row.querySelector("[data-tv-slide-name]");
        slide.name = nameInput && nameInput.value.trim() ? nameInput.value.trim() : (TV_LABELS[slide.id.split("-")[0]] || slide.id);
...
function renderTvManagement() {
    ...
    s.slides = tvNormalizeSlides(s).filter((slide) => {
        if (!slide || typeof slide.id !== "string") return false;
        const sourceId = slide.id.split("-")[0];
        return sourceId !== "photos" && sourceId !== "ranking";
    });
    ["attendanceVisit", "attendanceAr"].forEach((id) => {
        if (!s.slides.some((slide) => slide.id.split("-")[0] === id)) {
            s.slides.push({ id, name: TV_LABELS[id], enabled: true, duration: 15 });
        }
    });
    const p = tvPanel();
    if (!p) return;
    p.innerHTML = `
      <div class="bg-white rounded-[28px] p-5 sm:p-8 border border-slate-200 card-shadow space-y-7">
        ...
        <div class="flex flex-wrap gap-2 border-b pb-4 text-sm font-bold" id="tv-tabs">
          ${["slides:화면 순서", "content:문구·이미지", "appearance:화면 스타일", "status:연결 상태"].map((x, i) => { const [id, label] = x.split(":"); return `<button type="button" data-tv-tab="${id}" class="px-3 py-2 rounded-lg ${i ? "text-slate-500 hover:bg-slate-50" : "bg-indigo-50 text-indigo-700"}">${label}</button>`; }).join("")}
        </div>
        <div id="tv-tab-slides" class="tv-tab space-y-4"><div class="flex flex-wrap items-start justify-between gap-3"><div><h3 class="font-bold text-slate-800">표시할 화면과 순서</h3><p class="mt-1 text-sm text-slate-500">왼쪽 손잡이를 끌어 순서를 바꾸고, 화면 이름·노출 여부·유지 시간을 정하세요.</p></div><button type="button" data-tv-slide-add class="rounded-xl bg-indigo-600 px-4 py-2.5 text-sm font-bold text-white hover:bg-indigo-700">+ 화면 추가</button></div><div id="tv-slide-list" class="space-y-2">${s.slides.map((slide) => `<div draggable="true" data-tv-slide="${slide.id}" class="choice-btn rounded-2xl border border-slate-200 bg-white p-4 flex flex-wrap items-center gap-3 shadow-sm"><span class="cursor-move select-none text-xl text-slate-400" title="끌어서 순서 변경">⋮⋮</span><label class="min-w-[12rem] flex-1 text-sm font-bold text-slate-700">화면 이름 <input data-tv-slide-name type="text" value="${tvEscape(slide.name || TV_LABELS[slide.id.split("-")[0]] || slide.id)}" class="mt-1 w-full rounded-lg border border-slate-200 bg-white p-2 text-sm font-bold text-slate-800"></label><label class="flex items-center gap-2 text-sm font-bold text-slate-600"><input class="h-4 w-4 accent-indigo-600" type="checkbox" ${slide.enabled ? "checked" : ""}> TV에 표시</label><label class="flex items-center gap-2 text-sm text-slate-500">유지 시간 <select class="rounded-lg border border-slate-200 bg-white p-2 text-sm font-bold text-slate-700">${[5,8,10,15,20,30].map((n) => `<option value="${n}" ${slide.duration === n ? "selected" : ""}>${n}초</option>`).join("")}</select></label><button type="button" data-tv-duplicate="${slide.id}" class="text-sm font-bold text-indigo-600">복제</button><button type="button" data-tv-delete="${slide.id}" class="text-sm font-bold text-rose-600">삭제</button></div>`).join("")}</div></div>
        <div id="tv-tab-content" class="tv-tab hidden space-y-6">
          <div class="grid lg:grid-cols-2 gap-5">
          <section class="rounded-2xl border border-slate-200 bg-slate-50/70 p-5 space-y-4"><div><h3 class="font-bold text-slate-800">환영 화면</h3><p class="mt-1 text-xs text-slate-500">TV를 켰을 때 보이는 첫 인사말입니다.</p></div>${tvField("제목", `<input id="tv-welcome-title" value="${tvEscape(s.welcome.title)}" class="w-full rounded-xl border border-slate-200 bg-white p-3 text-sm" placeholder="예: 시흥시능곡청소년문화의집">`)}${tvField("부제목", `<input id="tv-welcome-subtitle" value="${tvEscape(s.welcome.subtitle)}" class="w-full rounded-xl border border-slate-200 bg-white p-3 text-sm" placeholder="예: 청소년의 꿈이 자라는 공간">`)}${tvField("추가 안내", `<textarea id="tv-welcome-description" class="w-full rounded-xl border border-slate-200 bg-white p-3 text-sm" rows="3" placeholder="필요한 경우에만 입력하세요">${tvEscape(s.welcome.description)}</textarea>`)}${tvField("로고 이미지", `<input id="tv-welcome-logo" value="${tvEscape(s.welcome.logo)}" class="w-full rounded-xl border border-slate-200 bg-white p-3 text-sm" placeholder="아래에서 파일을 선택하거나 이미지 주소를 붙여 넣으세요">`, "권장: 가로형 PNG 또는 JPG")}</section>
          <section class="rounded-2xl border border-indigo-100 bg-indigo-50/50 p-5"><div class="flex h-full flex-col"><div><h3 class="font-bold text-slate-800">방문 · AR 출석 이벤트</h3><p class="mt-2 text-sm leading-6 text-slate-600">기간 안에 조건을 만족한 이용자를 위한 이벤트입니다. 진행 상태와 방문자 화면 안내는 날짜를 기준으로 자동 처리됩니다.</p></div><button type="button" data-tv-attendance-add class="mt-4 w-full rounded-xl bg-indigo-600 px-4 py-3 text-sm font-bold text-white hover:bg-indigo-700">+ 출석 이벤트 만들기</button><div id="tv-attendance-events-editor" class="mt-4 grid gap-3"></div></div></section>
          </div>
          <div class="grid lg:grid-cols-2 gap-5"><section class="rounded-2xl border border-slate-200 bg-white p-5"><div class="flex items-start justify-between gap-3"><div><h3 class="font-bold text-slate-800">이벤트</h3><p class="mt-1 text-xs text-slate-500">이벤트를 만든 뒤 각 이벤트에 사진을 여러 장 추가할 수 있습니다.</p><p class="mt-1 text-xs font-semibold text-indigo-600">권장 이미지 규격: 1920 × 1080px (16:9)</p></div><button type="button" data-tv-content-add="events" class="rounded-lg bg-indigo-50 px-3 py-2 text-xs font-bold text-indigo-700">+ 이벤트 추가</button></div><div id="tv-event-upload-progress" class="mt-3 text-xs text-slate-500" aria-live="polite"></div><div id="tv-events-editor" class="mt-4 space-y-3"></div></section><section class="rounded-2xl border border-slate-200 bg-white p-5"><div class="flex items-start justify-between gap-3"><div><h3 class="font-bold text-slate-800">공지사항</h3><p class="mt-1 text-xs text-slate-500">이미지 공지는 TV 전체 화면에 포스터처럼 표시됩니다.</p></div><button type="button" data-tv-content-add="notices" class="rounded-lg bg-indigo-50 px-3 py-2 text-xs font-bold text-indigo-700">+ 텍스트 공지</button></div><label id="tv-notice-dropzone" class="mt-4 flex cursor-pointer flex-col items-center justify-center rounded-xl border-2 border-dashed border-slate-200 bg-slate-50 px-4 py-5 text-center hover:border-indigo-300 hover:bg-indigo-50/50"><span class="text-2xl">🖼️</span><span class="mt-2 text-sm font-bold text-slate-800">이미지 공지 선택</span><span class="mt-1 text-xs text-slate-500">포스터 이미지를 끌어놓거나 클릭하세요</span><span class="mt-1 text-xs font-semibold text-indigo-600">권장 규격: 1920 × 1080px (16:9)</span><input id="tv-notice-image-upload" type="file" accept="image/*" class="sr-only"></label><div id="tv-notice-preview" class="mt-3 hidden overflow-hidden rounded-xl border border-slate-200 bg-slate-50"><img alt="선택한 공지 이미지 미리보기" class="h-40 w-full object-cover"></div><button id="tv-notice-upload-button" type="button" disabled class="mt-3 w-full rounded-xl bg-indigo-600 px-4 py-3 text-sm font-bold text-white disabled:cursor-not-allowed disabled:opacity-40">Cloudinary에 업로드하고 등록</button><div id="tv-notice-upload-progress" class="mt-3 text-xs text-slate-500" aria-live="polite"></div><div id="tv-notices-editor" class="mt-4"></div></section></div>
        </div>
...
function tvBindManagement() {
    ...
    tvPanel().querySelector("[data-tv-slide-add]")?.addEventListener("click", () => {
        const options = Object.entries(TV_LABELS).map(([id, label]) => `${id} (${label})`).join("\n");
        const sourceId = prompt("추가할 화면 종류를 입력하세요.\n\n" + options, "welcome");
        if (!sourceId) return;
        const typedSource = sourceId.trim();
        const normalizedId = Object.keys(TV_LABELS).find((id) => id === typedSource.toLowerCase() || TV_LABELS[id] === typedSource);
        if (!TV_LABELS[normalizedId]) { showMessage("목록에 있는 화면 종류를 입력해 주세요."); return; }
        const name = prompt("TV에 표시할 화면 이름", TV_LABELS[normalizedId]);
        if (name === null) return;
        tvAdminSettings.slides.push({ id: normalizedId + "-" + Date.now(), name: name.trim() || TV_LABELS[normalizedId], enabled: true, duration: 8 });
        tvMarkDirty();
        renderTvManagement();
    });
    tvPanel().querySelectorAll("[data-tv-duplicate]").forEach((b) => b.addEventListener("click", () => { const source = tvAdminSettings.slides.find((s) => s.id === b.dataset.tvDuplicate); tvAdminSettings.slides.push({ ...source, id: source.id + "-" + Date.now(), name: (source.name || TV_LABELS[source.id.split("-")[0]] || "화면") + " 복사" }); tvMarkDirty(); renderTvManagement(); }));
...
function tvAttendanceRanking(event) {
    const source = event.type === "ar" ? arLogs : visitLogs;
...
async function tvEditAttendanceEvent(id) {
    try {
        ...
        const tvTitle = prompt("TV 순위표 제목 (선택, 비워두면 이벤트 이름 사용)", old.tvTitle || "") || "";
        const tvSubtitle = prompt("TV 순위표 부제·안내 문구 (선택, 비워두면 기존 안내 문구 사용)", old.tvSubtitle || "") || "";
        const tvEmptyMessage = prompt("TV 순위표 참여자가 없을 때 문구 (선택)", old.tvEmptyMessage || "") || "";
        ...
        await itemRef.set({
            ...
            tvSubtitle: tvSubtitle.trim(),
            tvEmptyMessage: tvEmptyMessage.trim(),
            enabled: true,
...
function tvRenderContent(type, items) {
    ...
    root.innerHTML = list.map(([id, item]) => {
        if (type === "events") {
            ...
            return `<div class="rounded-xl border border-slate-100 bg-slate-50/60 p-3 text-sm"><div class="min-w-0"><b class="block truncate text-slate-800">${tvEscape(item.title || "이벤트")}</b><div class="mt-1 text-xs text-slate-500">${tvEscape(item.startDate || "")} ~ ${tvEscape(item.endDate || "계속 표시")} · 사진 ${eventImages.length}장</div></div>${thumbnails ? `<div class="mt-3 grid grid-cols-3 gap-2">${thumbnails}</div>` : ""}<div class="mt-3 flex flex-wrap items-center gap-3 text-xs font-bold"><button type="button" data-tv-content-edit="${type}:${id}" class="text-indigo-600">수정</button><label class="cursor-pointer text-emerald-700">사진 여러 장 추가<input type="file" accept="image/*" multiple class="sr-only" data-tv-event-images="${id}"></label><span class="font-medium text-slate-500">권장 1920 × 1080px (16:9)</span><button type="button" data-tv-content-delete="${type}:${id}" class="text-rose-600">삭제</button></div></div>`;
        }
...
function tvRenderStatus() { const card = document.getElementById("tv-status-card"); if (!card) return; db.ref("tvStatus").once("value").then((s) => { const v = s.val() || {}; card.innerHTML = `<b class="text-slate-800">${v.online ? "● 온라인" : "● 오프라인"}</b><br>마지막 동기화: ${v.lastSync ? new Date(v.lastSync).toLocaleString("ko-KR") : "아직 없음"}<br>현재 화면: ${tvEscape(v.currentSlide || "-")}<br>재생목록: ${tvNormalizeSlides(tvAdminSettings).filter((x) => x.enabled).length}개 화면`; }); }
