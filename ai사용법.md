...
## 가장 중요한 원칙
가장 중요 가장 중요 가장 중요 가장 중요
1. 코드 수정이나 홈페이지 화면 수정에는 **GPT-5.6-Sol, 추론 수준 Low(낮음) 이상**을 사용하세요.
(2026년 7월 31일 기준으로는 유료 버전에서만 사용할 수 있는 기능입니다. 다만 추후 이 내용을 수정하실 시점에는 무료 버전에도 배포되어 사용 가능할 것으로 예상됩니다.. )
...
능곡 계정에는 권한이 전부 할당되어 있어걱정 권한문제는 걱정 안하셔도 됩니다 !
---
...
매우 중요한 부분입니다 !! 나쁜요청 예시처럼하면 정상적인 코드도 오류가 생깁니다 ㅠㅠ
---
...
이 문서는 개발 경험이 없는 분도 가능한 한 안전하게 홈페이지를 관리할 수 있도록 작성했습니다. 위 내용을 읽어도 이해하기 어렵거나, 수정 도중 예상하지 못한 오류가 발생했다면 더 이상 변경하거나 배포하지 말고 아래 이메일로 연락해 주세요.

choewonhyeog387@gmail.com


연락할 때는 가능하면 다음 내용을 함께 보내주세요.

- 문제가 발생한 화면의 전체 캡처
- 화면에 표시된 오류 문구
- `git status --short` 실행 결과
- AI가 수정했다고 보고한 파일 목록
- 운영 배포 전인지 배포 후인지

이 프로젝트는 장기간 안정적으로 운영할 수 있도록 중복 저장, 인증 전환, 네트워크 오류, TV 자동 복구와 같은 여러 상황을 고려하여 구현되어 있습니다. 다만 기능 추가나 운영 환경 변경에 따라 수정이 필요할 수 있으므로, 변경할 때는 반드시 이 문서의 절차를 따라주세요.

특히 일반 ChatGPT 웹사이트에 코드 일부만 복사해서 질문하는 방식은 권장하지 않습니다. 일부 코드만으로는 다른 파일과의 연결 관계, Firebase 데이터 구조, 기존 테스트와 운영 규칙을 확인할 수 없어 정상 기능까지 잘못 수정할 수 있습니다.

코드를 수정할 때는 반드시 다음 방식으로 진행해 주세요.

1. VS Code에서 `nchm_visiter` 프로젝트 폴더 전체를 엽니다.
2. VS Code에서 Codex를 사용할 수 있도록 준비합니다.
3. Codex가 프로젝트의 전체 파일 구조와 관련 코드를 확인할 수 있는 상태에서 요청합니다.
4. GPT-5.6-Sol, 추론 수준 Low 이상을 사용합니다 !! 꼭 지키셔야 합니다.
5. 수정 후 자동 테스트와 화면 검사를 모두 진행합니다.


잘 모르겠거나 판단하기 어려운 부분은 부담 갖지 말고 언제든 위 이메일로 문의해 주세요. 작은 질문이라도 괜찮습니다.
정말루..
